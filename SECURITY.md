...
| `qnReports` | — | — | full |
| `vendorQuotes` | get by direct link only (not listable); one-time update to `costs`/`vendorName`/`vendorNote`/`status`/`submittedAt` | — | full |
| `clientReports` | get by direct link only (not listable) | — | full |
